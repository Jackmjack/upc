...
MonthlyAttendanceWidget::MonthlyAttendanceWidget(int userId,QWidget *parent) :
    QWidget(parent),
    ui(new Ui::MonthlyAttendanceWidget)
{
    ...
    //创建标题标签
    QLabel *title = new QLabel("月度出勤",this);
    ...
    //连接按钮与槽函数
    connect(refresh,&QPushButton::clicked,this,&MonthlyAttendanceWidget::refreshTable);
...
void MonthlyAttendanceWidget::refreshTable()
{
    table->setRowCount(0);
    QSqlQuery query = db.executeSQL("SELECT id,staff_id,name,date,start_time,start_status,end_time,end_status FROM attendances WHERE staff_id = ?",{userId});
    int row = 0;
    while (query.next()) {
        table->insertRow(row);
