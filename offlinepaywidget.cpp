...
OfflinePayWidget::OfflinePayWidget(int userId,QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OfflinePayWidget)
{
    ui->setupUi(this);
    this->userId = userId;
    ...
    //创建标题标签
    QLabel *title = new QLabel("线下缴费", this);
    ...
    //创建输入框
    ownerNameEdit = new QLineEdit(this);
    ...
    ownerNameEdit->setPlaceholderText("请输入缴费人姓名");
    ownerIdEdit->setPlaceholderText("请输入要缴费的业主ID");
    ...
    formLayout->addRow("缴费人姓名：", ownerNameEdit);
    formLayout->addRow("业主ID：", ownerIdEdit);
    ...
    //连接按钮与槽函数
    connect(itemComboBox, &QComboBox::currentTextChanged, this, &OfflinePayWidget::calculatePay);
    connect(quantityEdit, &QLineEdit::textChanged, this, &OfflinePayWidget::calculatePay);
    connect(confirmBtn, &QPushButton::clicked, this, &OfflinePayWidget::confirmPayment);
...
void OfflinePayWidget::loadPaymentItems()
{
    itemComboBox->clear();
...
void OfflinePayWidget::calculatePay()
{
...
void OfflinePayWidget::confirmPayment()
{
    QString name = ownerNameEdit->text().trimmed();
    ...
    if (name.isEmpty() || ownerId.isEmpty() || itemName.isEmpty() || quantityStr.isEmpty() || amountStr.isEmpty()) {
        QMessageBox::warning(this, "错误", "请填写完整的缴费信息！");
        return;
    }
    QSqlQuery query = db.executeSQL("SELECT id FROM users WHERE id = ? AND role = 2",{ownerId});
    if(!query.next()) {
        QMessageBox::warning(this,"错误","未找到该业主！");
        return;
    }
    int itemId = itemComboBox->currentData().toInt();
    QString time = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    db.executeSQL("INSERT INTO paymentrecords (pay_name,owner_id,item_id,quantity,amount,pay_time,pay_method,operator_id) VALUES (?,?,?,?,?,?,0,?)",{name,ownerId.toInt(),itemId,quantityStr.toDouble(),amountStr.toDouble(),time,userId});
...
OfflinePayWidget::~OfflinePayWidget()
{
    delete ui;
}
